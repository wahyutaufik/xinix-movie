...
class oauth_server
{
	...
	public function validate_client($client_id = '', $client_secret = NULL, $redirect_uri = NULL)
	{
		...
		$client_check_query = $this->ci->db
										...
										->get_where('oauth_application', $params);
	...
	{		
		...
		else
		{
			...
			foreach ($scopes as $scope)
			{
				...
				{
					$this->ci->db
								->insert('oauth_session_scope', array(
			...
			if (count($scopes) > 0)
			{
				foreach ($scopes as $scope)
				{
					$scope_exists = $this->ci->db
												...
												->count_all_results('oauth_session_scope');
	...
	{
		$user = $this->ci->_model('user')->get_login(array('login' => $username, 'password' => $password));
		if (!empty($user)) {
			return (object) $user;
		}
		return false;
